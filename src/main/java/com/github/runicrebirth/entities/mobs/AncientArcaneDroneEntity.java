...
public class AncientArcaneDroneEntity extends Monster implements GeoEntity, SmartBrainOwner<AncientArcaneDroneEntity> {
    ...
    @Override
    public List<? extends ExtendedSensor<?>> getSensors(AncientArcaneDroneEntity entity) {
        return ObjectArrayList.of(new NearbyLivingEntitySensor<>(), new HurtBySensor<>());
    }

    @Override
    public ActivityBuilder<AncientArcaneDroneEntity> getCoreBehaviourGroup(AncientArcaneDroneEntity entity) {
        return ActivityBuilder.<AncientArcaneDroneEntity>create(Activity.CORE).behaviours(
            new LookAtTarget<>(),
            new StrafeTarget<>(),
    ...
    @Override
    public ActivityBuilder<AncientArcaneDroneEntity> getIdleBehaviourGroup(AncientArcaneDroneEntity entity) {
        return ActivityBuilder.<AncientArcaneDroneEntity>create(Activity.IDLE).behaviours(
            new FirstApplicableBehaviour<AncientArcaneDroneEntity>(
                new TargetOrRetaliate<>()
                    .useMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYER)
                    .canRetaliateAgainst(target -> target.isAlive() && (this == target.getLastHurtMob() ||
                        ((target instanceof Player player) && !player.getAbilities().invulnerable))),
                new SetPlayerLookTarget<>(),
                new SetRandomLookTarget<>()),
            new OneRandomBehaviour<>(
                new SetRandomWalkTarget<>().speedModifier(1),
                new Idle<>().runFor(entity2 -> entity2.getRandom().nextInt(30, 60))));
    }

    @Override
    public ActivityBuilder<AncientArcaneDroneEntity> getFightingBehaviourGroup(AncientArcaneDroneEntity entity) {
        return ActivityBuilder.<AncientArcaneDroneEntity>create(Activity.FIGHT).behaviours(
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<AncientArcaneDroneEntity>("idle_controller", 5, state ->
            state.setAndContinue(IDLE)));
        controllers.add(new AnimationController<AncientArcaneDroneEntity>("cast_controller", 2, state -> PlayState.STOP)
