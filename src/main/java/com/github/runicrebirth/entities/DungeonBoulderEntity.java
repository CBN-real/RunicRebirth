...
public class DungeonBoulderEntity extends Entity implements GeoEntity {
    ...
    protected void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput input) {
        String dirName = input.getStringOr("TravelDirection", "");
        if (!dirName.isEmpty()) {
            Direction d = Direction.byName(dirName);
            travelDirection = d != null ? d : Direction.NORTH;
    ...
    protected void addAdditionalSaveData(net.minecraft.world.level.storage.ValueOutput output) {
        output.putString("TravelDirection", travelDirection.getName());
    }

    @Override
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<DungeonBoulderEntity>("controller", 0,
