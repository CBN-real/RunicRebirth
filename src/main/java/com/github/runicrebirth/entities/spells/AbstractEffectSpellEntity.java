...
public abstract class AbstractEffectSpellEntity extends AbstractInstantSpellEntity {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<AbstractEffectSpellEntity>("spell_phase", 0, state -> {
            if (!this.isAddedToLevel()) {
                ...
                switch (displayPhase) {
                    case 0:
                        ...
                        if (currentTick - phaseStartTick > 0.5 && controller.getPlayState() == PlayState.STOP) {
                            displayPhase = 1;
