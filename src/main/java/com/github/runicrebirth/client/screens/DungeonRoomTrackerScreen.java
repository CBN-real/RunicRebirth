...
public class DungeonRoomTrackerScreen extends Screen {
    ...
    private void loadConfig(CompoundTag tag) {
        if (tag.contains("corner1")) corner1 = toRelative(BlockPos.of(tag.getLongOr("corner1", 0L)));
        if (tag.contains("corner2")) corner2 = toRelative(BlockPos.of(tag.getLongOr("corner2", 0L)));
        spawners.clear();
        if (tag.contains("spawners"))
            for (long l : tag.getLongArray("spawners").orElse(new long[0])) spawners.add(toRelative(BlockPos.of(l)));
        doors.clear();
        if (tag.contains("doors"))
            for (long l : tag.getLongArray("doors").orElse(new long[0])) doors.add(toRelative(BlockPos.of(l)));
        activations.clear();
        if (tag.contains("activation_blocks"))
            for (long l : tag.getLongArray("activation_blocks").orElse(new long[0])) activations.add(toRelative(BlockPos.of(l)));
    ...
    private void buildSettingsTab(int cx, int top) {
        ...
        addRenderableWidget(Button.builder(Component.literal("Reset Room"), b -> {
            ClientPacketDistributor.sendToServer(new DungeonRoomTrackerResetC2SPacket(trackerPos));
            onClose();
    ...
    public void extractRenderState(GuiGraphicsExtractor gfx, int mouseX, int mouseY, float partialTick) {
        extractBackground(gfx, mouseX, mouseY, partialTick);

        int cx = width / 2;

        gfx.centeredText(font, "Dungeon Room Tracker", cx, 3, 0xFFFFFF);
        gfx.fill(cx - 180, 33, cx + 180, 34, 0xFFAAAAAA);

        switch (currentTab) {
            case 0 -> {
                gfx.centeredText(font, "Zone Configuration", cx, 42, 0xFFFFAA);
                gfx.text(font, "C1: " + posToString(corner1), cx - 45, 198, 0xAAFFAA);
                gfx.text(font, "C2: " + posToString(corner2), cx - 45, 210, 0xAAFFAA);
            }
            case 1 -> gfx.centeredText(font, "Dungeon Mob Spawners (" + spawners.size() + ")", cx, 42, 0xFFFFAA);
            case 2 -> gfx.centeredText(font, "Dungeon Doors (" + doors.size() + ")", cx, 42, 0xFFFFAA);
            case 3 -> gfx.centeredText(font, "Activation Blocks (" + activations.size() + ")", cx, 42, 0xFFFFAA);
            case 4 -> {
                gfx.centeredText(font, "Settings", cx, 42, 0xFFFFAA);
                gfx.text(font, "Time bonus (seconds):", cx - 90, 62, 0xFFFFFF);
                gfx.text(font, "Resets room to INACTIVE and opens all doors.", cx - 90, 110, 0xFF6666);
            }
        }
    ...
    private void saveAndClose() {
        ...
        ClientPacketDistributor.sendToServer(new DungeonRoomTrackerSyncC2SPacket(trackerPos, config));
        onClose();
    }
