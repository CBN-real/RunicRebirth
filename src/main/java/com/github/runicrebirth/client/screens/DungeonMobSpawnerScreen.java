...
public class DungeonMobSpawnerScreen extends Screen {
    ...
    private void loadConfig(CompoundTag tag) {
        ...
        if (tag.contains("wave_entries")) {
            ...
            for (int i = 0; i < list.size(); i++)
                entries.add(DungeonMobSpawnerBlockEntity.MobWaveEntry.fromNbt(list.getCompoundOrEmpty(i)));
        }
        if (tag.contains("spawn_radius")) spawnRadius = tag.getFloatOr("spawn_radius", 0f);
    }
    ...
    private void addEntry() {
        try {
            ...
            entries.add(new DungeonMobSpawnerBlockEntity.MobWaveEntry(
                Identifier.parse(mobId), wave, count));
    ...
    private void saveAndClose() {
        ...
        ClientPacketDistributor.sendToServer(new DungeonMobSpawnerSyncC2SPacket(spawnerPos, tag));
        onClose();
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor gfx, int mouseX, int mouseY, float partialTick) {
        ...
        int cx = width / 2;
        gfx.centeredText(font, "Dungeon Mob Spawner", cx, 8, 0xFFFFFF);
        gfx.fill(cx - 130, 20, cx + 130, 21, 0xFFAAAAAA);
        // Row labels above input boxes
        gfx.text(font, "Wave:", cx - 110, INPUT_Y + 40, 0xAAAAAA);
        gfx.text(font, "Count:", cx - 56, INPUT_Y + 40, 0xAAAAAA);
        // Entries header
        if (!entries.isEmpty())
            gfx.text(font, "Entries (" + entries.size() + "):", cx - 170, LIST_TOP + 4, 0xFFFFAA);
        else
            gfx.centeredText(font, "No entries. Add mob wave entries above.", cx, LIST_TOP + 4, 0x888888);
        // Radius label
        gfx.text(font, "Spawn Radius:", cx - 95, RADIUS_Y + 3, 0xAAAAAA);
