package com.github.runicrebirth.client.renderers.models;

import com.github.runicrebirth.RunicRebirth;
import net.minecraft.resources.Identifier;
import com.geckolib.model.GeoModel;
import com.github.runicrebirth.entities.mobs.AncientArcaneDroneEntity;
import com.geckolib.renderer.base.GeoRenderState;

public class AncientArcaneDroneGeoModel extends GeoModel<AncientArcaneDroneEntity> {

    private static final Identifier MODEL =
        Identifier.fromNamespaceAndPath(RunicRebirth.MODID, "entity/arcane_drone");
    private static final Identifier TEXTURE =
        Identifier.fromNamespaceAndPath(RunicRebirth.MODID, "textures/entity/runic_templates/arcane_runic_template.png");
    private static final Identifier ANIMATIONS =
        Identifier.fromNamespaceAndPath(RunicRebirth.MODID, "entity/arcane_drone");

    @Override
    public Identifier getModelResource(GeoRenderState renderState) { return MODEL; }

    @Override
    public Identifier getTextureResource(GeoRenderState renderState) { return TEXTURE; }

    @Override
    public Identifier getAnimationResource(AncientArcaneDroneEntity animatable) { return ANIMATIONS; }

    // TODO GeckoLib 5: setCustomAnimations removed. Migrate body bone rotation to
    // renderer's adjustModelBonesForRender(RenderPassInfo, BoneSnapshots) via a custom GeoRenderState.
}
