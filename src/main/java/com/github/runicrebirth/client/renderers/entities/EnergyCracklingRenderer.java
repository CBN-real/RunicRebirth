...
public class EnergyCracklingRenderer extends EntityRenderer<EnergyCracklingEntity, EnergyCracklingRenderer.State> {

    private static final int BASE_BOLT_COUNT = 7;
    private static final float BASE_TICKS_PER_UPDATE = 2f;

    public static class State extends EntityRenderState {
        public float radius, density, speed, thickness, length;
        public int colorRgb;
    ...
    @Override
    public void extractRenderState(EnergyCracklingEntity entity, State state, float partialTicks) {
        super.extractRenderState(entity, state, partialTicks);
        state.radius = entity.getCrackleRadius();
        state.density = entity.getDensity();
        state.speed = entity.getSpeed();
        state.thickness = entity.getThickness();
        state.length = entity.getCrackleLength();
        state.colorRgb = entity.getColor();
    ...
    public void submit(State state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera) {
        ...
        float thickness = state.thickness;
        float length    = state.length;
        int colorRgb    = state.colorRgb;
        ...
        long seed = ((long) (state.tickCount / ticksPerUpdate)) * 0x9E3779B97F4A7C15L ^ (long) state.entityId;

        int boltCount = Math.max(1, Math.round(BASE_BOLT_COUNT * density));

        final float rf = r, gf = g, bf = b;
            ...
            if (length > 0) {
                ...
                for (int i = 0; i < boltCount; i++) {
                    ...
                    Vec3 end = new Vec3(
                        ...
                        (rand.nextDouble() - 0.5) * length,
                        az + (rand.nextDouble() - 0.5) * lateralSpread);
                    drawBoltCylinder(consumer, mat, start, end, rf, gf, bf, rand, thickness, lateralSpread);
                }
            } else {
                for (int i = 0; i < boltCount; i++) {
                    if (rand.nextFloat() < 0.25f) continue;
                    ...
                    drawBolt(consumer, mat, start, end, rf, gf, bf, rand, thickness);
                }
            }
        });
