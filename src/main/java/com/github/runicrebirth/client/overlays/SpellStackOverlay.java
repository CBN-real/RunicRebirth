 ...
 */
public class SpellStackOverlay implements GuiLayer {
    ...
    private static final Identifier SLOT_BIG_EMPTY =
        Identifier.fromNamespaceAndPath("runicrebirth", "hud/overlay_slot_border");
    private static final Identifier SLOT_SMALL_FILLED =
        Identifier.fromNamespaceAndPath("runicrebirth", "hud/overlay_slot_border_small");
    private static final Identifier SLOT_SMALL_UNAVAIL =
        Identifier.fromNamespaceAndPath("runicrebirth", "hud/overlay_slot_border_small_unavail");
    private static final Identifier SLOT_SELECTED =
        Identifier.fromNamespaceAndPath("runicrebirth", "hud/overlay_slot_border_selected");
    private static final Identifier CHARGE_ICONS =
        Identifier.fromNamespaceAndPath("runicrebirth", "textures/gui/sprites/hud/charge_icons.png");
    ...
    private static final Identifier LOCK_ICON =
        Identifier.fromNamespaceAndPath("runicrebirth", "textures/gui/locked_spell_overlay.png");
    ...
    public void render(GuiGraphicsExtractor graphics, DeltaTracker delta) {
        ...
        for (int i = 0; i < miniCount; i++) {
            ...
            graphics.pose().translate((float)miniX, (float)miniY);
            graphics.pose().scale(MINI_SCALE, MINI_SCALE);
    ...
    private static Identifier elementForEntry(WandStacksData wandData, int idx) {
    ...
    private static void drawCooldowns(GuiGraphicsExtractor graphics) {
        Map<Identifier, Integer> remaining = ClientMagicData.cooldownRemaining();
        ...
        int i = 0;
        for (Map.Entry<Identifier, Integer> entry : remaining.entrySet()) {
            ...
            graphics.blitSprite(RenderPipelines.GUI_TEXTURED, SLOT_BIG_EMPTY, x, y, COOLDOWN_SIZE, COOLDOWN_SIZE);
            graphics.blit(RenderPipelines.GUI_TEXTURED, type.getSpellIconPath(),
                x + COOLDOWN_ICON_OFFSET, y + COOLDOWN_ICON_OFFSET,
                0f, 0f, COOLDOWN_ICON_SIZE, COOLDOWN_ICON_SIZE, COOLDOWN_ICON_SIZE, COOLDOWN_ICON_SIZE, COOLDOWN_ICON_SIZE, COOLDOWN_ICON_SIZE);
    ...
    private static void drawStackRow(GuiGraphicsExtractor g, List<SpellComponent> stack,
                                     int rowLeftX, int rowTopY, boolean isActive, int maxSmallSlots,
                                     Identifier elementId, WandStacksData.StackEntry rawEntry,
                                     int initialCharges) {
        ...
        // Big slot background
        Identifier bigSlotSprite;
        if (typeInStack != null) {
            Identifier elemId = elementId != null ? elementId
                : (typeInStack.defaultElement() != null ? typeInStack.defaultElement().id() : null);
            String elemPath = elemId != null ? elemId.getPath() : "arcane";
            bigSlotSprite = Identifier.fromNamespaceAndPath(RunicRebirth.MODID, "hud/" + elemPath + "_overlay_slot_border");
        } else {
            bigSlotSprite = SLOT_BIG_EMPTY;
        }
        g.blitSprite(RenderPipelines.GUI_TEXTURED, bigSlotSprite, rowLeftX, rowTopY, BIG_SIZE, BIG_SIZE);

        // Big icon
        if (typeInStack != null) {
            g.blit(RenderPipelines.GUI_TEXTURED, typeInStack.getSpellIconPath(),
                rowLeftX + BIG_ICON_OFFSET,
                rowTopY + BIG_ICON_OFFSET,
                0f, 0f, BIG_ICON, BIG_ICON, BIG_ICON, BIG_ICON, BIG_ICON, BIG_ICON);
            if (rawEntry.inscribed() && rawEntry.hasPermanentSpellType()) {
                g.blit(RenderPipelines.GUI_TEXTURED, LOCK_ICON, rowLeftX + BIG_SIZE - LOCK_SIZE - 1, rowTopY + 1,
                    0f, 0f, LOCK_SIZE, LOCK_SIZE, 32, 32, 32, 32);
            }
        }

        // Selected border (only for active stack)
        if (isActive) {
            g.blitSprite(RenderPipelines.GUI_TEXTURED, SLOT_SELECTED,
        ...
        for (int i = 0; i < maxSmallSlots; i++) {
            ...
            if (i < modifiers.size()) {
                ...
                g.blitSprite(RenderPipelines.GUI_TEXTURED, mod.getOverlaySlotPath(), x, y, SMALL_SIZE, SMALL_SIZE);
                g.blit(RenderPipelines.GUI_TEXTURED, mod.getSpellIconPath(), x, y, 0f, 0f, SMALL_ICON, SMALL_ICON, SMALL_ICON, SMALL_ICON, SMALL_ICON, SMALL_ICON);
                if (rawEntry.inscribed() && isModifierPermanent(rawEntry, mod)) {
                    int lockS = LOCK_SIZE - 2;
                    g.blit(RenderPipelines.GUI_TEXTURED, LOCK_ICON, x + SMALL_SIZE - lockS, y,
                        0f, 0f, lockS, lockS, 32, 32, 32, 32);
                }
            } else {
                g.blitSprite(RenderPipelines.GUI_TEXTURED, SLOT_SMALL_UNAVAIL, x, y, SMALL_SIZE, SMALL_SIZE);
            }
        }
        ...
        if (isActive && totalCasts > 1) {
            ...
            if (remaining > 0) {
                ...
                g.blit(RenderPipelines.GUI_TEXTURED, CHARGE_ICONS, largeX, largeY,
                    (float) uMid, 0f, 9, 9, CHARGE_ICON_SMALL, CHARGE_ICON_SMALL, CHARGE_TEX_W, CHARGE_TEX_H);

                // Left (small, 7x7)
                int uLeft = filledCount >= 2 ? uFilled : uEmpty;
                g.blit(RenderPipelines.GUI_TEXTURED, CHARGE_ICONS, leftSmallX, smallY,
                    (float) uLeft, 0f, 6, 6, CHARGE_ICON_SMALL, CHARGE_ICON_SMALL, CHARGE_TEX_W, CHARGE_TEX_H);

                // Right (small, 7x7)
                int uRight = filledCount >= 3 ? uFilled : uEmpty;
                g.blit(RenderPipelines.GUI_TEXTURED, CHARGE_ICONS, rightSmallX, smallY,
                    (float) uRight, 0f, 6, 6, CHARGE_ICON_SMALL, CHARGE_ICON_SMALL, CHARGE_TEX_W, CHARGE_TEX_H);
            }
        }
    }
}
