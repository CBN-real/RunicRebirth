...
public class SpellRingOverlay implements GuiLayer {
    ...
    public void render(GuiGraphicsExtractor graphics, DeltaTracker delta) {
        ...
        for (int i = 0; i < MAX_RINGS; i++) {
            ...
            if (!stack.isEmpty()) {
                graphics.fakeItem(stack, x + 1, slotY + 1);
    ...
    private static void drawSlotBackground(GuiGraphicsExtractor graphics, int x, int y, boolean filled) {
    ...
    private static void drawKeyLabel(GuiGraphicsExtractor graphics, Minecraft mc, int x, int y, int slotIndex) {
        ...
        graphics.text(mc.font, keyName, 0, 0, 0xFFFFFF, false);
        pose.popMatrix();
    }

    private static void drawDurationBar(GuiGraphicsExtractor graphics, ItemStack stack, int x, int y) {
        Identifier durId = getDurationId(stack);
    ...
    private static void drawCooldownBar(GuiGraphicsExtractor graphics, ItemStack stack, int x, int y) {
        Identifier cdId = getCooldownId(stack);
        if (cdId == null) return;
    ...
    private static Identifier getDurationId(ItemStack stack) {
    ...
    private static Identifier getCooldownId(ItemStack stack) {
