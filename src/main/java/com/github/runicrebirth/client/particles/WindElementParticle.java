...
public class WindElementParticle extends SimpleAnimatedParticle {

    
    private final boolean mirrored;

    public WindElementParticle(ClientLevel level, double x, double y, double z,
                               SpriteSet spriteSet, double xd, double yd, double zd, float scale) {
        super(level, x, y, z, spriteSet, 0.0f);
        this.xd = xd;
        this.yd = yd;
        this.zd = zd;
        this.scale((this.random.nextFloat() + 1.5f) * scale);
    ...
    public void tick() {
        super.tick();
        this.xd += this.random.nextFloat() / 50.0F * (this.random.nextBoolean() ? 1 : -1);
        this.yd += this.random.nextFloat() / 50.0F * (this.random.nextBoolean() ? 1 : -1);
        this.zd += this.random.nextFloat() / 50.0F * (this.random.nextBoolean() ? 1 : -1);
    ...
    public static class Provider implements ParticleProvider<ScaledParticleOption> {
        ...
        public Particle createParticle(ScaledParticleOption type, ClientLevel level,
            ...
            return new WindElementParticle(level, x, y, z, this.sprites, dx, dy, dz, type.getScale());
        }
    }
}
