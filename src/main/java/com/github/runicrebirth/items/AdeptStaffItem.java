...
public class AdeptStaffItem extends SpellWriter implements GeoItem, IMagicStaff {
    ...
    public AdeptStaffItem(Properties properties) {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<AdeptStaffItem>("controller", 5, state -> {
    ...
    public void createGeoRenderer(Consumer<GeoRenderProvider> consumer) {
        consumer.accept(new GeoRenderProvider() {
            private GeoItemRenderer<AdeptStaffItem> renderer;

            @Override
            public com.geckolib.renderer.GeoItemRenderer<?> getGeoItemRenderer() {
                if (this.renderer == null) {
                    this.renderer = new com.github.runicrebirth.client.renderers.items.AdeptStaffRenderer();
                }
