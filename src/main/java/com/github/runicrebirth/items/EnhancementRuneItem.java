...
public class EnhancementRuneItem extends Item {
    ...
    private final Identifier runeTypeId;
    private final int tier;

    public EnhancementRuneItem(Identifier runeTypeId, int tier, Properties properties) {
    ...
    @Override
    public void appendHoverText(ItemStack stack, TooltipContext context, net.minecraft.world.item.component.TooltipDisplay display, java.util.function.Consumer<Component> tooltips, TooltipFlag flag) {
        int tierIdx = Math.min(tier, 3);
        tooltips.accept(Component.literal(TIER_NAMES[tierIdx]).withStyle(TIER_COLORS[tierIdx]));

        CompoundTag stats = stack.get(ModDataComponents.RUNE_STATS.get());
        if (stats == null || stats.isEmpty()) {
            tooltips.accept(Component.literal("Place in inventory to roll stats").withStyle(ChatFormatting.DARK_GRAY));
            return;
        }
        for (String key : stats.keySet()) {
            float val = stats.getFloat(key).orElse(0f);
            String displayKey = formatStatKey(key);
            String displayVal = formatStatValue(key, val);
            tooltips.accept(Component.literal(displayKey + ": " + displayVal).withStyle(ChatFormatting.GRAY));
        }
    }
