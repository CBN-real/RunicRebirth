...
public final class ShapeRegistry {
    ...
    public record Shape(ResourceLocation id, List<StrokePoint> template, double threshold, Supplier<SpellComponent> component) {}

    private static final Map<ResourceLocation, Shape> SHAPES = new HashMap<>();

    private ShapeRegistry() {}

    public static void register(ResourceLocation id, List<StrokePoint> template, double threshold, Supplier<SpellComponent> component) {
    ...
    public static SpellComponent componentFor(ResourceLocation id) {
        Shape s = SHAPES.get(id);
        return s == null ? null : s.component().get();
    }

    public static double thresholdFor(ResourceLocation id) {
        Shape s = SHAPES.get(id);
